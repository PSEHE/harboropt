...
##### TO-DO: 
##### 3. For EE and solar and nondisp resources: Incorporate avoided transmission and distribution costs. Need to value additional energy generated (avoided generation, t&d costs, and monetized grid emissions). Need to input actual values for marginal costs of generation and t&d.
##### 6. Incorporate demand response.
##### 4. Optimize just among clean energy resources? And compare to a separate case where gas is built?
...
##### 1. Inflation —— make sure all costs are in correct dollar year ($2018). NREL ATB projected costs use 2.5% inflation rate. Need to incorporate inflation rate for EE costs. Need to confirm that inflation costs are already incorporated into projected fixed and variable costs from NREL.
...
##### 8. Split up results functions into specific functions (ex. gen_fractions, curtailment).
...
##### 12. Discounting rate for each build year currently projects costs only to end of 30 year timespan. Change to 30-year timespan for each resource from whenever it's built?
        
class LinearProgram(object):
    
    def __init__(self, initial_state_of_charge = 0, storage_lifespan = 15, portfolio_timespan = 30, storage_can_charge_from_grid = True,
        discount_rate = 0.03, cost=1, build_years = 1, health_cost_range = 'HIGH', cost_projections = 'moderate', build_start_year =2020,
        transmission_cost_per_mw = 49250, storage_resilience_incentive_per_kwh = 1000, resilient_storage_grid_fraction = 0.7,
        social_cost_carbon_short_ton = 46.3, avoided_marginal_generation_cost_per_mwh = 0.50, 
        avoided_marginal_transmission_cost_per_mwh = 0.50, avoided_marginal_distribution_cost_per_mwh = 0.50): 
#         diesel_genset_carbon_per_mw = 2, diesel_genset_pm25_per_mw = 2, diesel_genset_nox_per_mw = 2, diesel_genset_so2_per_mw = 2, 
#         diesel_genset_pm10_per_mw = 2, diesel_genset_fixed_cost_per_mw_year = 35000, diesel_genset_mmbtu_per_mwh =0,
#         diesel_genset_cost_per_mmbtu = 20, diesel_genset_hours_per_year = 24):#,gas_fuel_cost=4):
...
#         self.diesel_genset_hours_per_year = diesel_genset_hours_per_year
        ...
        if self.health_costs == 'HIGH':
            ...
            print(self.pm25_cost_short_ton_la)
            ...
            print(self.so2_cost_short_ton_la)
            ...
            print(self.nox_cost_short_ton_la)
...
#                         max_discharge.SetCoefficient(discharge, -1)
                ...
                #Still within hourly loop, calculate value of excess energy generated.
                #value_additional_energy = self.calculate_value_additional_energy()
            ...
            for resource in self.disp.index:
                ...
                capex_cost_per_mw = self.resource_costs.loc[capex_cost_inds & resource_inds, str(cost_year)].item()*1000
                ...
                fixed_per_mw_year_extrapolated = self.resource_costs.loc[fixed_cost_inds & resource_inds, str(cost_year)].item()*1000 * self.discounting_factor[year]
                
                #Add fixed costs accumulated over chosen timespan to lcoe dictionary.
                self.lcoe_dict[year][resource]['fixed_extrapolated']=fixed_per_mw_year_extrapolated
                
                capex_fixed = capex_cost_per_mw + fixed_per_mw_year_extrapolated
            ...
            for resource in self.storage.index:
                ...
                if self.storage.loc[resource, 'resilient'] == 'y':
                    ...
                    capex_per_mw = capex_per_mw -  incentive_per_mw
                    if capex_per_mw < 0:
                         capex_per_mw = 0
                    
                    fixed = self.resource_costs.loc[fixed_cost_inds & resource_inds, str(cost_year)].item()*1000 * self.discounting_factor[year]
...
#                     capex_per_mw =  capex_per_mw - monetized_emissions_saved - fuel_costs_saved
#                     fixed = fixed - diesel_genset_fixed_cost_yearly
                ...
                number_of_storage_replacements = int((self.timespan -1)/ self.storage_lifespan)

                ## Calculate replacement capex costs in future years, apply discount rate, and add to original capex cost.
                growth_rate = 1.0 + self.discount_rate
                for i in range(number_of_storage_replacements):
                    ...
                    discount_factor = pow(growth_rate, -(storage_replacement_year-self.build_start_year))
                ...
                objective.SetCoefficient(self.storage_capacity_vars[resource][year], capex_per_mw + fixed)
            ...
            for resource in self.nondisp.index: 
                ...
                capex = self.resource_costs.loc[capex_cost_inds & resource_inds, str(cost_year)].item()*1000
                ...
                if not resource_weighted_avg_eul_inds.empty:
                    ...
                    number_of_replacements = int((self.timespan -1)/ weighted_avg_eul)
                    ...
                    for i in range(number_of_replacements):
                        ...
                        #Calculate discounting factor to apply to capex in the given replacement year.
                        discount_factor = pow(growth_rate, -(replacement_year-self.build_start_year))
                        replacement_capex_discounted =  replacement_capex * discount_factor

                        capex = capex + replacement_capex_discounted
               
                
                #Add capex cost for given build year to lcoe dictionary.
                self.lcoe_dict[year][resource]['capex']=capex
                ...
                resource_monetized_emissions = (self.nondisp.loc[resource, 'co2_short_tons_per_mwh']*self.social_cost_carbon_short_ton) + self.nondisp.loc[resource, 'nox_lbs_per_mwh']*self.nox_cost_short_ton_la + self.nondisp.loc[resource, 'so2_lbs_per_mwh']*self.so2_cost_short_ton_la 
               #+ self.nondisp.loc[resource, 'pm10_lbs_per_mwh']*self.pm10_cost_per_ton + self.nondisp.loc[resource, 'pm25_lbs_per_mwh']*self.pm25_cost_per_ton

                #Sum annual generation for each unit of capacity. Extrapolate to timespan, accounting for discount rate.
                variable_cost_inds = self.resource_costs['cost_type']=='variable_per_mwh'
                
                annual_sum_var_cost = (self.resource_costs.loc[variable_cost_inds & resource_inds, str(cost_year)].item()+resource_monetized_emissions) * profile_sum
                ...
                self.lcoe_dict[year][resource]['variable_extrapolated']=self.resource_costs.loc[variable_cost_inds & resource_inds, str(cost_year)].item() * profile_sum * self.discounting_factor[year]
